...
class Clio:
    """
    Main interface for Clio - Apiarist Client Library -
...
class SessionProtocol(ResourceMixin):
    ...
    expected_filters = ('protocol', 'source_ip',
                        'source_port', 'destination_ip',
                        'destination_port', 'honeypot', '_id')


class RawEvent(ResourceMixin):

    collection_name = 'raw_events'
    expected_filters = ('ident', 'channel',
                        'payload', '_id',
                        'timestamp', )

    channel_map = {
        'snort.alerts': ['date', 'sensor',
                         'source_ip', 'destination_port',
                         'priority', 'classification',
                         'signature'],
        'dionaea.capture': ['url', 'daddr',
                            'saddr', 'dport',
                            'sport', 'sha512', 'md5'],
        'glastopf.events': ['time', 'pattern',
                            'filename', 'source',
                            'request_url'],
        'suricata.events': ['timestamp', 'sensor',
                            'source_ip', 'destination_port',
                            'proto', 'signature'],
    }
    ...
    def get_payloads(self, options, req_args):
        ...
        cnt_query = super(RawEvent, self)._clean_query(req_args)
...
class File(ResourceMixin):
    ...
    expected_filters = ('_id', 'content_guess',
                        'encoding', 'hashes',)
...
class AuthKey(ResourceMixin):
    ...
    expected_filters = ('identifier', 'secret',
                        'publish', 'subscribe',
                        'name', 'created_date',
                        'ip', 'honeypot',
                        'uuid', '_id', )
    ...
    def get_sensors(self):
        sensors = list(self.collection.find({}, {'identifier': 1,
                                                 'name': 1,
                                                 'created_date': 1,
                                                 'ip': 1,
                                                 'hostname': 1,
        ...
        for sensor in sensors:
            sensor['attacks_count'] = Clio().counts.get_count(identifier=sensor['uuid'])
        return sensors
