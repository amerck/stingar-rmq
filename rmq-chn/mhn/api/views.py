...
@token_auth
def get_sensors():
    return _get_query_resource(Clio().authkey, request.args.to_dict())
...
def get_sensor(uuid):
    sensor = {}
    results = Clio().authkey.get(identifier=uuid)
    if results:
        sensor = results.to_dict()
    return jsonify(
        data=sensor,
        meta={
            'size': len(sensor),
...
def get_feed(feed_id):
    return _get_one_resource(Clio().raw_event, feed_id)
...
@token_auth
def get_feeds():
    return _get_query_resource(Clio().raw_event, request.args.to_dict())
