...
def dashboard():
    ...
    # TOP 5 sigs
    freq_sigs = clio.raw_event.top_sigs(top=5, hours_ago=24)
...
def get_attacks():
    clio = Clio()
    sensors = clio.authkey.get_sensors()
    ...
    print("Mongo Pages Sessions:", sessions)
    return render_template('ui/attacks.html', attacks=sessions,
                           sensors=sensors, view='ui.get_attacks',
...
def get_feeds():
    ...
    count, columns, feeds = clio.raw_event.get_payloads(options, request.args.to_dict())
    channel_list = clio.raw_event.channel_map.keys()
...
def get_sensors():
    clio = Clio()
    sensors = clio.authkey.get_sensors()
    total = clio.authkey.get_sensor_count()
    # Paginating the list.
...
def get_credentials_payloads(clio):
    ...
    credentials_payloads += clio.raw_event.get_payloads({'limit': 10000}, {"channel": "kippo.sessions"})[2]
    credentials_payloads += clio.raw_event.get_payloads({'limit': 10000}, {"channel": "cowrie.sessions"})[2]
...
def graph_passwords():
    ...
    clio = Clio()
    top_passwords = clio.raw_event.count_passwords(get_credentials_payloads(clio))
...
def graph_users():
    ...
    clio = Clio()
    top_users = clio.raw_event.count_users(get_credentials_payloads(clio))
...
def graph_combos():
    ...
    clio = Clio()
    top_combos = clio.raw_event.count_combos(get_credentials_payloads(clio))
