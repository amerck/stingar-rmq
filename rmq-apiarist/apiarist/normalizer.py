...
class Normalizer(object):
    ...
    def start_processing(self, fetch_count=1500):
        ...
        while self.enabled:
            ...
            to_be_processed = self.database.get_event_data(oldest_id, fetch_count)
            to_be_inserted = []

            for event in to_be_processed:
                try:
                    channel = event['channel']
                    if event['_id'] < oldest_id:
                        oldest_id = event['_id']
                    if channel in self.normalizers:
                        norm = self.normalizers[channel].normalize(event['payload'],
                                                                   channel, event['timestamp'],
                                                                   ignore_rfc1918=self.ignore_rfc1918)

                        # batch up normalized items
                        to_be_inserted.append((norm, event['_id'], event['ident']))
                        normalizations += 1
                    elif channel not in no_normalizers_warnings:
                        error_list.append({'_id': event['_id'],
                    ...
                    logger.warning('Failed to normalize and import item with hpfeed id = {0}, channel = {1}. ({2}). '
                                   ...
                                   .format(event['_id'], event['channel'], err))

            if len(error_list) > 0:
                self.database.apiarist_set_errors(error_list)

            if len(to_be_inserted):
                print("Inserting!")
                self.inserter(to_be_inserted)
    ...
    def inserter(self, to_be_inserted):
        for norm, obj_id, identifier in to_be_inserted:
            self.database.insert_normalized(norm, obj_id, identifier)
